...
const mapDoctor = (d) => (d ? { ...d, name: t(d.name), nameEn: t(d.nameEn), spec: t(d.spec), dept: t(d.dept), bodyHtml: cleanHtml(d.bodyHtml) } : d);
...
const DOC = `{ "slug": slug.current, name, nameEn, "spec": coalesce(specialization, department), "dept": department, "photo": photo.asset->url, bodyHtml }`;
...
export async function getDoctorsByDepartments(names = []) {
  ...
  return sanity
    .fetch(`*[_type=="doctor" && department in $names && defined(slug.current)]|order(name asc)${DOC}`, { names })
    .then(many(mapDoctor));
}
...
export async function getDiseasesByCategories(cats = []) {
  ...
  return sanity
    .fetch(`*[_type=="disease" && category in $cats && defined(slug.current)]|order(title asc)${DIS}`, { cats })
...
export async function search(query) {
  ...
  const [doctors, diseases, news] = await Promise.all([
    sanity
      .fetch(
        `*[_type=="doctor" && defined(slug.current) && (name match $w || nameEn match $w || specialization match $w || department match $w)]|order(name asc)[0...40]${DOC}`,
        { w }
      )
      .then(many(mapDoctor)),
    sanity
      .fetch(
        `*[_type=="disease" && defined(slug.current) && (title match $w || category match $w || excerpt match $w)]|order(title asc)[0...60]${DIS}`,
        { w }
      )
      .then(many(mapDisease)),
    sanity
      .fetch(
        `*[_type=="news" && defined(slug.current) && (title match $w || excerpt match $w)]|order(date desc)[0...30]${NEWS}`,
        { w }
      )
      .then(many(mapNews)),
  ]);
