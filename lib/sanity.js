...
//
// Отдельно вырезаем формы. В текстах, перенесённых с WordPress, попадаются
// целые формы заявок со старого сайта: они собирают имя, телефон и почту,
// но отправляются на несуществующий адрес, то есть заявка пропадает.
// Вдобавок у них нет согласия на обработку данных. Настоящая форма на сайте
// одна — наша, с проверкой и согласием; всё остальное из статей убираем.
function cleanHtml(html) {
  ...
  return stripEmoji(normalizeHeadings(html))
    ...
    .replace(/<\/?(?:input|textarea|select|option|button|fieldset|legend|label|form)\b[^>]*>/gi, "")
    .replace(/\s(?:style|class|id|align|border|cellpadding|cellspacing|bgcolor)="[^"]*"/gi, "")
    .replace(/\s(?:width|height)="[^"]*"/gi, "")
    .replace(/<\/?(?:span|font)[^>]*>/gi, "")
    .replace(/<p>(?:\s|&nbsp;|<br\s*\/?>)*<\/p>/gi, "")
    // Пункт списка в импорте часто выглядит так:
    //   <li><strong>Название</strong>: <ul>…</ul></li>
    // Название пункта мы выводим отдельной строкой (см. .cms-body li > strong
    // в globals.css), и двоеточие после него оставалось висеть само по себе —
    // на узком экране это выглядело как строка из одного знака. Переносим его
    // внутрь названия, чтобы оно осталось на той же строке.
    .replace(/<\/strong>\s*:\s*(?=<(?:ul|ol|br|div|p)\b|<\/li>)/gi, ":</strong>")
    .replace(/(?:&nbsp;)+/g, " ")
...
// Помимо специальности раздел может забирать врачей поимённо — по slug.
// Нужно там, где специальность в базе одна, а на сайте врач должен попасть
// и в смежный раздел: гематолог с пометкой «Онкологи» и так далее.
// В базе при этом ничего не дублируется.
export async function getDoctorsByDepartments(names = [], slugs = []) {
  if (!names.length && !slugs.length) return [];
  return sanity
    .fetch(
      `*[_type=="doctor" && defined(slug.current) && (department in $names || slug.current in $slugs)]|order(name asc)${DOC_LIST}`,
      { names, slugs }
    )
    .then(many(mapDoctor));
}

// Заболевания направления. Та же логика: категория у статьи одна, поэтому
// статьи вроде «Лейкоз» добираются к гематологии по slug, оставаясь в онкологии.
export async function getDiseasesByCategories(cats = [], slugs = []) {
  if (!cats.length && !slugs.length) return [];
  return sanity
    .fetch(
      `*[_type=="disease" && defined(slug.current) && (category in $cats || slug.current in $slugs)]|order(title asc)${DIS_LIST}`,
      { cats, slugs }
    )
